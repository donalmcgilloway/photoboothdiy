...
Numeral = "1"  # Numeral is the number display
Message = "Hello"  # Message is a fullscreen message
BackgroundColor = "green"
CountDownPhoto = ""
CountPhotoOnCart = ""
SmallMessage = "hello 2"  # SmallMessage is a lower banner message
TotalImageCount = 3  # Counter for Display and to monitor paper usage
PhotosPerCart = 30  # Selphy takes 16 sheets per tray
imagecounter = 0
imagefolder = '../images/'
templatePath = os.path.join('Photos', 'Template', "template_new.png") #Path of template image
photoOnePath = os.path.join('Photos', 'Screens', "1.jpg") #Path of template image
ImageShowed = False
Printing = True
debugMode = False
BUTTON_PIN = 25
...
def InitFolder():
    ...
    imagefolder2 = os.path.join(imagefolder, '../images/originals/')
...
def CapturePicture():
	...
	for x in range(3, -1, -1):
        ...
        ts = time.time()
        filename = os.path.join(imagefolder, 'originals', str(imagecounter)+"_"+str(ts) + '.jpg')
...
def TakePictures():
	...
	global TotalImageCount
        ...
        bgimage.paste(image1, (890, 30))
        bgimage.paste(image2, (890, 605))
        bgimage.paste(image3, (60, 605))
        # Create the final filename
        ts = time.time()
        Final_Image_Name = os.path.join(imagefolder, "Final_" + time.strftime("%Y-%m-%d-%H-%M-%S") +"_"+str(ts) + ".jpg")
        ...
        if Printing:
                if (debugMode == True):
                 ...
                 #       if os.path.isfile('/home/pi/Desktop/tempprint.jpg'):
                                ...
                                UpdateDisplay()
                                cups.setUser('pi')
                                conn.printFile(default_printer, '/home/pi/Desktop/tempprint.jpg', "Photobooth" + time.strftime("%Y-%m-%d-%H-%M-%S"),{})
                                #time.sleep(40)
                else:
                         Message = "Printing in progress..."
                         conn = cups.Connection()
                         ...
                         conn.printFile(default_printer, '/home/pi/Desktop/tempprint.jpg', "Photobooth" + time.strftime("%Y-%m-%d-%H-%M-%S"),{})
                         #time.sleep(40)
                        #Message = "We're not able to print, but we'll send you on your photos!"
                        #Numeral = ""
                        #UpdateDisplay()
                        #time.sleep(1)
...
def WaitForEvent():
    ...
    for event in pygame.event.get():  # Hit the ESC key to quit the slideshow.
       if (event.type == QUIT or
           (event.type == KEYDOWN and event.key == K_ESCAPE)):
           pygame.quit()
       elif (event.type == MOUSEBUTTONUP): # Touchscreen or mousebutton up to take picture
           TakePictures()
           time.sleep(0.2)
